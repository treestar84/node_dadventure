<template>
  <div class="social-hub">
    ...
    <div class="social-header">
      <h3 class="section-title">👥 소셜</h3>
      <div class="social-stats">
        <div class="stat-item">
          <span class="stat-icon">👫</span>
          <span class="stat-text">친구 {{ friends.length }}명</span>
        </div>
        <div class="stat-item">
          <span class="stat-icon">🚪</span>
          <span class="stat-text">방문 {{ visitStats.totalVisitsGiven }}회</span>
        </div>
      </div>
    </div>
    ...
    <div class="social-tabs">
      ...
      >
        👫 친구
        <span v-if="onlineFriends.length > 0" class="online-indicator">{{ onlineFriends.length }}</span>
      </button>
      <button 
        @click="activeTab = 'visits'"
        :class="['tab-button', { active: activeTab === 'visits' }]"
      >
        📖 방문 기록
      </button>
    </div>
    ...
    <div v-if="activeTab === 'discover'" class="tab-content">
      ...
      <div class="search-bar">
        <input
          ...
          class="search-input"
        />
        <button @click="refreshDiscoverable" class="refresh-button">
    ...
    <div v-if="activeTab === 'friends'" class="tab-content">
      <div v-if="friends.length === 0" class="empty-state">
        <p>👋 아직 친구가 없습니다</p>
        <p class="hint">탐색 탭에서 다른 캐릭터들을 찾아보세요!</p>
      </div>
